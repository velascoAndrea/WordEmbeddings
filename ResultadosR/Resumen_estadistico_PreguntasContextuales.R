...
resumen_f1_contextuales <- preguntasContextuales %>%
  group_by(Modelo) %>%
  summarise(
    Total_Preguntas = n(),
...
print(resumen_f1_contextuales)

# Resumen estadístico para Escala de Likert
resumen_likert_contextuales <- preguntasContextuales %>%
  group_by(Modelo) %>%
  summarise(
...
print(resumen_likert_contextuales)
